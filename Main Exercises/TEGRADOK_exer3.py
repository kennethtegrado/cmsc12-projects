...
def isEven(num1): #define a function named isEven and place a single input parameter
    ...
    return (num1 % 2 == 0) #function will return True if the number placed modulo 2 is equal to 0, otherwise False
...
def isOdd(num2): #define a function named isOdd and place a single input parameter
    ...
    return (num2 % 2 != 0) #function will return True if number placed modulo 2 is not equal to 0 , otherwise False
...
def isPositive(num1, num2): #define a function named isPositive and place two input parameter
    ...
    return (num1 > 0) and (num2 > 0) #return True if both integers placed are positive numbers, otherwise False
...
def sumSquares(ranges): #define a function named sumSquares and place a single input parameter
    ...
    for num in range(1, ranges + 1): #iterate between the elements starting from 1 up to the value of the ranges parameter
...
def createBox(num): #define a function named createBox with a single input parameter
    ...
    return boxHolder #return the value of string containing the syntax for generating a box to the program
...
def oddAverage(num): #define a function named oddAverage with a single input parameter
    ...
    average = 0 #create a variable average to store the computed value of (sumOdd / totalNumber), set temporary value to 0
    for i in range(1, num + 1): #iterate between the elements starting from one until the end value with an additional 1 in it
    ...
    average = sumOdd / totalNumber #compute for the average by dividing the sumOdd by totalNumber then store its value to average variable
...
def actionChecker(num1, num2): #define a function named actionChecker that can accept two input parameters 
    ...
    if isEven(num1) and isOdd(num2) and isPositive(num1, num2): 
        ...
        if not isPositive(num1, num2): #condition to check if both integers are positive numbers by calling the isPositive() function
            ...
            print('x and y must be both POSITIVE integers!') #prompt the user that the input should be both POSITIVE integers
...
def showMenu(): #define a new function that will be named as menu
    ...
    print("Please choose from the following:\n 1. Summation\n 2. Box\n 3. Average\n 0. Exit") #print a statement to show the operations available
...
while action: #a loop that allows us to continuously ask the user what operation they want to achieve from using the program
    ...
    if userChoice == '1': #condition that will check if the user chose the first option which is summation
        ...
        sum = sumSquares(firstNumber) #call the sumSquares() function then set the parameter to the value of the first integer
        #store the value of the summation in the sum variable so that we can put a prompt telling what the value is
        print('The sum of the squares of all even integers from 1 to', firstNumber, 'is', sum) #prompt to tell us what the value is
        ...
        box = createBox(secondNumber) #call the createBox() function then set the parameter to the value of the second integer
        ...
        average = oddAverage(firstNumber) #call the oddAverage() function then set the parameter to the value of the first integer
        #then store the returned value of the oddAverage() function in the average variable so that we can print the value
        print('The average of odd integers from 1 to', firstNumber, 'is', average)
        print() #add a line space after this instance of the loop
        ...
        print('Goodbye! Have a great day!') #prompt to inform the user that the program is now exiting
